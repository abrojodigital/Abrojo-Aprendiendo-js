...
const archivoDataRandom = _ => {
  ...
  let modelo = modelos[aleatorioMarca][Math.floor(Math.random()*(modelos[aleatorioMarca].length))]
  let aleatorioSeguro = tipoSeguros[Math.floor(Math.random()*(tipoSeguros.length))]
...
const archivoDirectorioRandomico = async (directorioActual) => {
  if (archivosCreados < archivosMaximosACrear) {
    ...
    if (random < 0.80) {
      ...
      const archivoRandom = 'auto'+crypto.randomBytes(parseInt(Math.random() * 20) + 1).toString('hex')
      ...
      if (random < 0.90 && profundidad < maximaProfundidadDirectorios) {
        const directorioRandom = crypto.randomBytes(parseInt(Math.random() * 11)+1).toString('hex')
    ...
    process.stdout.cursorTo(0)
    console.log(`Creados ${archivosMaximosACrear} archivos de pólizas de autos`)
  }
}
